...
// Step 2: monthly Compliance Assistant message allowance. Free users
// have 0; values are roughly 2× the plan's aiFixesPerMonth cap so the
// chatbot and AI fixes share a similar monthly envelope.
...
export async function POST(request: Request) {
  ...
  if (limit === 0) {
    return Response.json(
      { error: 'Compliance Assistant requires a Starter plan or higher', code: 'UPGRADE_REQUIRED' },
      { status: 403 }
    )
  }
